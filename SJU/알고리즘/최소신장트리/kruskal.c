...
GRAPH *kruskalMST(GRAPH *graph) {
	KRQ *queue = init_queue(graph);
	KDS *disjSet = init_disjointset(graph); 
	GRAPH *MST = spanning_subgraph(graph);
...
EDGE *removeMin(GRAPH *graph, KRQ *queue) {
	...
	for (int i = 0; i < graph->numOfedge; i++) {
		if (queue[i].edge != NULL)
			if (queue[i].edge->weight < min_ret->weight) { 
				min_ret = queue[i].edge;
				index = i;
			}
	}

	queue[index].edge = NULL;
	return min_ret;
}
